...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if response.status_code == 200:
            ...
            satellites_data = json_data[:10]
            sat_obj = []
            for satellite_data in satellites_data:
                sat_obj.append(satellite_data['OBJECT_ID'][:-1])
                
            satellites_to_create = []

            for i in sat_obj:
                new_url="https://celestrak.org/NORAD/elements/gp.php?INTDES="+i+"&FORMAT=json"
                new_response = requests.get(new_url)
                sat_resp = new_response.json()             
                   
                for satellite_data in sat_resp:
                    satellite = Satellite(
